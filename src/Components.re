...
module BaseGameObject = (G: {type t;}) => {
  ...
  let makeBase = (scene, name) => VervEffect(PBaseGO.makeBase(scene, name));
  ...
  let parentContainer = container =>
    container |> flatMap(PBaseGO.parentContainer);
  let ignoreDestroy = container =>
    container |> flatMap(PBaseGO.ignoreDestroy);

  let getIndexList = container => container |> flatMap(PBaseGO.getIndexList);
  let getData = container => container |> flatMap(PBaseGO.getData);
  let removeInteractive = container =>
    container |> map(PBaseGO.removeInteractive);
  let setName = (name, container) =>
    container |> map(PBaseGO.setName(_, name));
  let setData = (data, container) =>
    container |> map(PBaseGO.setData(_, data));
  let willRender = (camera2D, container) =>
    container |> flatMap(PBaseGO.willRender(_, camera2D));
  let setState = (state, container) =>
    container |> map(PBaseGO.setState(_, state));
  let setActive = (active, container) =>
    container |> map(PBaseGO.setActive(_, active));
...
module Alpha = (A: {type t;}) => {
  ...
  let alphaTopRightF = container =>
    container |> flatMap(PAlpha.alphaTopRightF);
  let alphaBottomLeft = container =>
    container |> flatMap(PAlpha.alphaBottomLeft);
  let alphaBottomLeftF = container =>
    container |> flatMap(PAlpha.alphaBottomLeftF);
  let alphaBottomRight = container =>
    container |> flatMap(PAlpha.alphaBottomRight);
  let alphaBottomRightF = container =>
    container |> flatMap(PAlpha.alphaBottomRightF);
  let clearAlpha = container => container |> map(PAlpha.clearAlpha);

  let setAlpha =
      (~topLeft=?, ~topRight=?, ~bottomLeft=?, ~bottomRight=?, container) => {
    ...
    | (Some(topLeft), Some(topRight), Some(bottomLeft), Some(bottomRight)) =>
      container
      |> map(fn(_, ~topLeft, ~topRight, ~bottomLeft, ~bottomRight, ()))
    | (None, Some(topRight), Some(bottomLeft), Some(bottomRight)) =>
      container |> map(fn(_, ~topRight, ~bottomLeft, ~bottomRight, ()))
    | (None, None, Some(bottomLeft), Some(bottomRight)) =>
      container |> map(fn(_, ~bottomLeft, ~bottomRight, ()))
    | (None, None, None, Some(bottomRight)) =>
      container |> map(fn(_, ~bottomRight, ()))
    | (None, Some(topRight), None, Some(bottomRight)) =>
      container |> map(fn(_, ~topRight, ~bottomRight, ()))
    | (None, Some(topRight), None, None) =>
      container |> map(fn(_, ~topRight, ()))
    | (None, Some(topRight), Some(bottomLeft), None) =>
      container |> map(fn(_, ~topRight, ~bottomLeft, ()))
    | (None, None, Some(bottomLeft), None) =>
      container |> map(fn(_, ~bottomLeft, ()))
    | (Some(topLeft), None, None, None) =>
      container |> map(fn(_, ~topLeft, ()))
    | (Some(topLeft), None, Some(bottomLeft), None) =>
      container |> map(fn(_, ~topLeft, ~bottomLeft, ()))
    | (Some(topLeft), Some(topRight), None, None) =>
      container |> map(fn(_, ~topLeft, ~topRight, ()))
    | (Some(topLeft), Some(topRight), None, Some(bottomRight)) =>
      container |> map(fn(_, ~topLeft, ~topRight, ~bottomRight, ()))
    | (Some(topLeft), Some(topRight), Some(bottomLeft), None) =>
      container |> map(fn(_, ~topLeft, ~topRight, ~bottomLeft, ()))
    | (Some(topLeft), None, Some(bottomLeft), Some(bottomRight)) =>
      container |> map(fn(_, ~topLeft, ~bottomLeft, ~bottomRight, ()))
    | (Some(topLeft), None, None, Some(bottomRight)) =>
      container |> map(fn(_, ~topLeft, ~bottomRight, ()))
...
module ScrollFactor = (SF: {type t;}) => {
  ...
  let setScrollFactor = (~x, ~y=?, container) =>
    switch (y) {
...
module Transform = (T: {type t;}) => {
  ...
  let setScale = (~x, ~y=?, container) =>
    switch (y) {
    | Some(y) => container |> map(T.setScale(_, ~x, ~y, ()))
    | None => container |> map(T.setScale(_, ~x, ()))
    };
  let setPosition = (~x=0, ~y=?, ~z=0, ~w=0, container) =>
    switch (y) {
    | Some(y) => container |> map(T.setPosition(_, ~x, ~y, ~z, ~w, ()))
    | None => container |> map(T.setPosition(_, ~x, ~y=x, ~z, ~w, ()))
    };
  let setRotation = (~radians=0, container) =>
    container |> map(T.setRotation(_, radians));
  let setRotationF = (~radians=0.0, container) =>
    container |> map(T.setRotationF(_, radians));

  let getParentRotation = container =>
    container |> flatMap(T.getParentRotation);
  let getParentRotationF = container =>
    container |> flatMap(T.getParentRotationF);
};
...
module Depth = (D: {type t;}) => {
  ...
  let setDepth = (depth, container) =>
    container |> map(PDepth.setDepth(_, depth));
};
...
module Origin = (O: {type t;}) => {
  ...
  let setDisplayOrigin = (~x=0.0, ~y=?, container) =>
    switch (y) {
    | Some(y) => container |> map(O.setDisplayOrigin(_, ~x, ~y, ()))
    | None => container |> map(O.setDisplayOrigin(_, ~x, ~y=x, ()))
    };
  let setOrigin = (~x=0.5, ~y=?, container) =>
    switch (y) {
    | Some(y) => container |> map(O.setOrigin(_, ~x, ~y, ()))
    | None => container |> map(O.setOrigin(_, ~x, ~y=x, ()))
    };

  let setOriginFromFrame = container =>
    container |> map(O.setOriginFromFrame);
  let updateDisplayOrigin = container =>
    container |> map(O.updateDisplayOrigin);
};
module ComputedSize = (CS: {type t;}) => {
  ...
  let setSize = (~width, ~height, container) =>
    container |> map(CS.setSize(_, ~width, ~height));
  let setSizeF = (~width, ~height, container) =>
    container |> map(CS.setSizeF(_, ~width, ~height));
  let setDisplaySize = (~width, ~height, container) =>
    container |> map(CS.setDisplaySize(_, ~width, ~height));
  let setDisplaySizeF = (~width, ~height, container) =>
    container |> map(CS.setDisplaySizeF(_, ~width, ~height));
};
...
module Size = (S: {type t;}) => {
  ...
  let setSize = (~width, ~height, container) =>
    container |> map(S.setSize(_, ~width, ~height));
  let setSizeF = (~width, ~height, container) =>
    container |> map(S.setSizeF(_, ~width, ~height));
  let setDisplaySize = (~width, ~height, container) =>
    container |> map(S.setDisplaySize(_, ~width, ~height));
  let setDisplaySizeF = (~width, ~height, container) =>
    container |> map(S.setDisplaySizeF(_, ~width, ~height));
  let setSizeToFrame = (~frame, container) =>
    container |> map(S.setSizeToFrame(_, ~frame));
};
...
module Tint = (T: {type t;}) => {
  ...
  let tintBottomLeft = container =>
    container |> flatMap(PTint.tintBottomLeft);
  let tintBottomRight = container =>
    container |> flatMap(PTint.tintBottomRight);
  ...
  let setTintFill =
    ...
    | (None, Some(topRight), Some(bottomLeft), Some(bottomRight)) =>
      container
      |> map(PTint.setTintFill(_, ~topRight, ~bottomLeft, ~bottomRight, ()))
    | (None, None, Some(bottomLeft), Some(bottomRight)) =>
      container |> map(PTint.setTintFill(_, ~bottomLeft, ~bottomRight, ()))
    | (None, None, None, Some(bottomRight)) =>
      container |> map(PTint.setTintFill(_, ~bottomRight, ()))
    | (None, Some(topRight), None, Some(bottomRight)) =>
      container |> map(PTint.setTintFill(_, ~topRight, ~bottomRight, ()))
    | (None, Some(topRight), None, None) =>
      container |> map(PTint.setTintFill(_, ~topRight, ()))
    | (None, Some(topRight), Some(bottomLeft), None) =>
      container |> map(PTint.setTintFill(_, ~topRight, ~bottomLeft, ()))
    | (None, None, Some(bottomLeft), None) =>
      container |> map(PTint.setTintFill(_, ~bottomLeft, ()))
    | (Some(topLeft), None, None, None) =>
      container |> map(PTint.setTintFill(_, ~topLeft, ()))
    | (Some(topLeft), None, Some(bottomLeft), None) =>
      container |> map(PTint.setTintFill(_, ~topLeft, ~bottomLeft, ()))
    | (Some(topLeft), Some(topRight), None, None) =>
      container |> map(PTint.setTintFill(_, ~topLeft, ~topRight, ()))
    | (Some(topLeft), Some(topRight), None, Some(bottomRight)) =>
      container
      |> map(PTint.setTintFill(_, ~topLeft, ~topRight, ~bottomRight, ()))
    | (Some(topLeft), Some(topRight), Some(bottomLeft), None) =>
      container
      |> map(PTint.setTintFill(_, ~topLeft, ~topRight, ~bottomLeft, ()))
    | (Some(topLeft), None, Some(bottomLeft), Some(bottomRight)) =>
      container
      |> map(PTint.setTintFill(_, ~topLeft, ~bottomLeft, ~bottomRight, ()))
    | (Some(topLeft), None, None, Some(bottomRight)) =>
      container |> map(PTint.setTintFill(_, ~topLeft, ~bottomRight, ()))
  ...
  let setTint =
    ...
    | (None, Some(topRight), Some(bottomLeft), Some(bottomRight)) =>
      container
      |> map(PTint.setTint(_, ~topRight, ~bottomLeft, ~bottomRight, ()))
    | (None, None, Some(bottomLeft), Some(bottomRight)) =>
      container |> map(PTint.setTint(_, ~bottomLeft, ~bottomRight, ()))
    | (None, None, None, Some(bottomRight)) =>
      container |> map(PTint.setTint(_, ~bottomRight, ()))
    | (None, Some(topRight), None, Some(bottomRight)) =>
      container |> map(PTint.setTint(_, ~topRight, ~bottomRight, ()))
    | (None, Some(topRight), None, None) =>
      container |> map(PTint.setTint(_, ~topRight, ()))
    | (None, Some(topRight), Some(bottomLeft), None) =>
      container |> map(PTint.setTint(_, ~topRight, ~bottomLeft, ()))
    | (None, None, Some(bottomLeft), None) =>
      container |> map(PTint.setTint(_, ~bottomLeft, ()))
    | (Some(topLeft), None, None, None) =>
      container |> map(PTint.setTint(_, ~topLeft, ()))
    | (Some(topLeft), None, Some(bottomLeft), None) =>
      container |> map(PTint.setTint(_, ~topLeft, ~bottomLeft, ()))
    | (Some(topLeft), Some(topRight), None, None) =>
      container |> map(PTint.setTint(_, ~topLeft, ~topRight, ()))
    | (Some(topLeft), Some(topRight), None, Some(bottomRight)) =>
      container
      |> map(PTint.setTint(_, ~topLeft, ~topRight, ~bottomRight, ()))
    | (Some(topLeft), Some(topRight), Some(bottomLeft), None) =>
      container
      |> map(PTint.setTint(_, ~topLeft, ~topRight, ~bottomLeft, ()))
    | (Some(topLeft), None, Some(bottomLeft), Some(bottomRight)) =>
      container
      |> map(PTint.setTint(_, ~topLeft, ~bottomLeft, ~bottomRight, ()))
    | (Some(topLeft), None, None, Some(bottomRight)) =>
      container |> map(PTint.setTint(_, ~topLeft, ~bottomRight, ()))
...
module Pipeline = (P: {type t;}) => {
  ...
  let defaultPipeline = container =>
    container |> flatMap(PPipeline.defaultPipeline);
  let pipeline = container => container |> flatMap(PPipeline.pipeline);

  let getPipelineName = container =>
    container |> flatMap(PPipeline.getPipelineName);
  let resetPipeline = container =>
    container |> flatMap(PPipeline.resetPipeline);
  let initPipeline = (~pipelineName, container) =>
    container |> map(PPipeline.initPipeline(_, ~pipelineName));
  let setPipeline = (~pipelineName=?, container) =>
    switch (pipelineName) {
    | Some(name) =>
      container |> map(PPipeline.setPipeline(_, ~pipelineName=name, ()))
